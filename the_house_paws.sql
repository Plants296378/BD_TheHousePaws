...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 25-11-2024 a las 15:43:14
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Empleado_Mascota` ()   BEGIN

SELECT 
	cit.cit_ID AS cita_id,
    cit.cit_motivo AS motivo,
    cit.cit_fecha AS fecha,
    em.emple_nombre AS nombre_del_encargado,
    masc.masct_nom AS nombre_Mascota
FROM 
        cita AS cit
    INNER JOIN 
        mascota AS masc ON cit.cit_Mascota_ID = masc.masct_ID
    INNER JOIN 
        empleado AS em ON cit.cit_empleado_ID = em.emple_id;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `recibo_mascota` (IN `p_recibo_id` INT)   BEGIN
    SELECT 
        r.rec_ID AS Recibo_ID,
        r.rec_fecha_emision AS Fecha_Emision,
        r.rec_monto AS Monto,
        masct.masct_nom AS Mascota
    FROM RECIBO r
    INNER JOIN mascota masct ON r.rec_Cliente_ID = masct.masct_Cliente_ID
    WHERE r.rec_ID = p_recibo_id;
END$$
...
--
ALTER TABLE `historial_clinico`
  MODIFY `hc_ud` int(11) NOT NULL AUTO_INCREMENT;
